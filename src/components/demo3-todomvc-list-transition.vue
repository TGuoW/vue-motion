<template>
  <section class="todoapp">
    ...
    <section class="main">
      ...
      <TransitionMotion
        :default-styles="getDefaultStyles()"
        :styles="getStyles"
        :will-leave="willLeave"
        :will-enter="willEnter">
        <template slot-scope="props">
          <ul class="todo-list">
            <li
              ...
              :class="item.data.isDone ? 'completed' : ''">
              <div class="view">
                <input
                  :checked="item.data.isDone"
                  class="toggle"
                  type="checkbox"
                  @change="handleDone(item.key)"
                >
                <label>{{ item.data.text }}</label>
                <button
                  class="destroy"
                  @click="handleDestroy(item.key)"
                />
              </div>
            </li>
          </ul>
      ...
      <ul class="filters">
        ...
        <li>
          <a
            ...
            @click="handleSelect('active')">
            Active
          </a>
        </li>
        <li>
          <a
            :class="selected === 'completed' ? 'selected' :''"
            @click="handleSelect('completed')">
            Completed
          </a>
        </li>
      </ul>
...
export default {
  ...
  computed: {
    itemsLeft () {
      return this.todos.filter(({data: {isDone}}) => !isDone).length
    },
    getStyles () {
  ...
  methods: {
    ...
    handleSubmit (e) {
      ...
      const newItem = {
        key: 't' + Date.now(),
        data: {text: this.value, isDone: false},
      }
      // append at head
      this.todos = [newItem].concat(this.state.todos)
    },
    ...
    handleToggleAll () {
      const allNotDone = this.todos.every(({data}) => data.isDone)
    ...
    handleClearCompleted () {
      this.todos = this.todos.filter(({data}) => !data.isDone)
    },

    handleDestroy (date) {
      this.todos = this.todos.filter(({key}) => key !== date)
    },
